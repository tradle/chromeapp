...
function App() {
  ...
  keeperConf.dht = new DHT({
    bootstrap: keeperConf.bootstrap,
  ...
  this.keeper.on('ready', function () {
    console.log('keeper ready, torrent port: ' + self.keeper.torrentPort());
  ...
  this.bitjoe.on('ready', function () {
    ...
    // console.log('My first few public key(s):\n');
    // var l = self.bitjoe.wallet().addresses.external.length;
    // var addresses = self.bitjoe.wallet().deriveAddresses(numToPrint);
    // var numToPrint = Math.min(addresses.length, 3);
    // for (var i = 0; i < numToPrint; i++) {
    //   console.log(self.bitjoe.getPublicKeyForAddress(addresses[i]).toHex());
    // }
